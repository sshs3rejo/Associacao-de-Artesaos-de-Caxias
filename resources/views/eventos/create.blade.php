...
<div class="max-w-4xl mx-auto my-10 bg-white rounded-2xl shadow-lg px-8 py-10">
    <h1 class="text-2xl font-bold text-brand mb-6">Cadastrar Novo Evento</h1>

    {{-- Exibe erros de validação --}}
    @if ($errors->any())
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg mb-6 relative" id="err-create-evento">
    ...
    <form action="{{ route('eventos.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <x-input name="nome" label="Nome do Evento" placeholder="Digite o nome do evento" required />

        <x-textarea name="descricao" label="Descrição" placeholder="Descreva o evento" rows="4" required />

        {{-- Linha 1: Tipo / Data Inicio / Data Fim --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <x-select name="tipo_evento" label="Tipo do Evento" :options="['feira' => 'Feira', 'exposicao' => 'Exposição', 'workshop' => 'Workshop', 'lancamento' => 'Lançamento', 'palestra' => 'Palestra', 'outro' => 'Outro']" placeholder="Selecione..." required />
            </div>
            <div>
                <x-input name="data_inicio" label="Data e Hora de Início" type="datetime-local" required />
            </div>
            <div>
                <x-input name="data_fim" label="Data e Hora de Fim" type="datetime-local" required />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
            <div class="md:col-span-4">
                <x-input name="local" label="Local" placeholder="Ex: Auditório Central" required />
            </div>
            <div class="md:col-span-3">
                <x-input name="capacidade_maxima" label="Capacidade Máx." type="number" required />
            </div>
            <div class="md:col-span-2">
                <x-input name="valor_inscricao" label="Valor (R$)" type="number" step="0.01" value="{{ old('valor_inscricao', '0.00') }}" required />
            </div>
            <div class="md:col-span-3">
                <x-select name="status" label="Status" :options="['planejado' => 'Planejado', 'confirmado' => 'Confirmado', 'em_andamento' => 'Em Andamento', 'concluido' => 'Concluído', 'cancelado' => 'Cancelado']" required />
            </div>
        </div>
        ...
        <div class="flex justify-between items-center">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 bg-brand hover:bg-brand-light text-white rounded-lg font-semibold transition duration-200">
                <x-icon name="check-circle" class="w-4 h-4" /> Salvar Evento
            </button>
        </div>
    </form>
</div>
