...
@section('content')
<style>
    /* Reaproveita o mesmo estilo do create */
    .evento-container {
        max-width: 900px;
        margin: 40px auto;
        background: #fff;
    ...
    <form action="{{ route('eventos.update', $evento->id_evento) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="nome_evento" name="nome_evento"
                   value="{{ old('nome_evento', $evento->nome_evento) }}" required>
        </div>

        {{-- Descrição --}}
        <div class="mb-3">
            <label for="descricao" class="form-label">Descrição</label>
            <textarea class="form-control" id="descricao" name="descricao" rows="4" required>{{ old('descricao', $evento->descricao) }}</textarea>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="time" class="form-control" id="horario_evento" name="horario_evento"
                       value="{{ old('horario_evento', $evento->horario_evento) }}" required>
            </div>
            <div class="col-md-4 mb-3">
                ...
                <input type="number" class="form-control" id="vagas_disponiveis" name="vagas_disponiveis"
                       value="{{ old('vagas_disponiveis', $evento->vagas_disponiveis) }}" required>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="local_evento" name="local_evento"
                   value="{{ old('local_evento', $evento->local_evento) }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="id_instrutor" name="id_instrutor" required>
                ...
                @foreach ($instrutores as $instrutor)
                    <option value="{{ $instrutor->id_instrutor }}" {{ old('id_instrutor', $evento->id_instrutor) == $instrutor->id_instrutor ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            @if ($evento->imagem)
                ...
                <img src="{{ asset($evento->imagem) }}" alt="Imagem atual do evento" class="img-preview">
            @endif
        </div>
