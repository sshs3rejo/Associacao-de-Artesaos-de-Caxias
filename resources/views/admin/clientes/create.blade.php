...
<div class="max-w-4xl mx-auto my-10 bg-white rounded-2xl shadow-lg px-8 py-10">
    <h1 class="text-2xl font-bold text-brand mb-6">Novo Cliente</h1>
    ...
    <form action="{{ route('admin.clientes.store') }}" method="POST">
        ...
        <x-input name="nome" label="Nome completo" placeholder="Ex: João Santos" required />

        <x-input name="email" label="Email" type="email" placeholder="Ex: joao@email.com" />
        ...
        <div class="flex justify-between items-center">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 bg-brand hover:bg-brand-light text-white rounded-lg font-semibold transition duration-200">
                <x-icon name="check-circle" class="w-4 h-4" /> Salvar Cliente
            </button>
        </div>
    </form>
</div>
