                            ...
                            <td class="p-3 text-sm text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.oficinas.destroy', $oficina->id) }}" method="POST" class="m-0">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-block px-3 py-1 rounded-lg font-semibold text-center no-underline border border-red-500 text-red-600 hover:bg-red-50 text-sm" onclick="return confirm('Tem certeza de que deseja excluir a oficina {{ $oficina->nome }}?')">
