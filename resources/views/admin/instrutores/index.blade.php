...
<div class="w-full px-4 py-5">
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('admin.instrutores.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-brand hover:bg-brand-light text-white rounded-lg font-semibold text-sm transition no-underline">
            <x-icon name="plus" class="w-4 h-4" /> Novo Instrutor
        </a>
    </div>
                            ...
                            <td class="p-3 text-sm text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.instrutores.destroy', $instrutor->id_instrutor) }}" method="POST" class="m-0">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="inline-block px-3 py-1 rounded-lg font-semibold text-center no-underline border border-red-500 text-red-600 hover:bg-red-50 text-sm" onclick="var f=this.closest('form'); showConfirm('Tem certeza de que deseja excluir o instrutor {{ $instrutor->nome }}?',function(){f.submit();}); return false;">
