...
@section('titulo', 'Editar Artesão')

@section('content')
<div class="max-w-4xl mx-auto my-10 bg-white rounded-2xl shadow-lg px-8 py-10">
    <h1 class="text-2xl font-bold text-brand mb-6">Editar Artesão: {{ $user->name }}</h1>
    ...
    <form action="{{ route('admin.artesao.update', $user) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <x-input name="specialty" label="Especialidade" :value="$user->artisanProfile?->specialty ?? ''" placeholder="Ex: Cerâmica Artesanal" />

        <x-textarea name="bio" label="Biografia" :value="$user->artisanProfile?->bio ?? ''" placeholder="Conte um pouco sobre o artesão..." rows="3" />

        <div class="mb-4">
            <label class="inline-flex items-center gap-2">
                <input type="hidden" name="is_public" value="0">
                <input type="checkbox" name="is_public" value="1" {{ $user->artisanProfile?->is_public ? 'checked' : '' }} class="rounded border-gray-300 text-brand focus:ring-brand-light">
        ...
        <div class="flex justify-between items-center">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 bg-brand hover:bg-brand-light text-white rounded-lg font-semibold transition duration-200">
                <x-icon name="check-circle" class="w-4 h-4" /> Atualizar Artesão
            </button>
        </div>
    </form>
</div>
